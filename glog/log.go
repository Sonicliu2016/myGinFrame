...
var (
	...
	levelPrefix = [LevelDebug + 1]string{"[Emergency]", "[Alert]", "[Critical]", "[Error]", "[Warn]", "[Notice]", "[Info]", "[Debug]"}
)
...
type BeeLogger struct {
	lock                sync.Mutex
	init                bool
	enableFuncCallDepth bool //是否输出文件名和行号
	enableFullFilePath  bool //是否获取打印文件绝对路径
	enableRelativePath  bool //是否获取打印文件相对路径
	asynchronous        bool
	wg                  sync.WaitGroup
	level               int
...
func (bl *BeeLogger) writeMsg(lm *LogMsg) error {
	...
	lm.enableRelativePath = bl.enableRelativePath
...
func EnableRelativePath(b bool) {
	beeLogger.enableRelativePath = b
}
